...
# created for Interspeech 2020
# coded by Bagus Tris Atmaja (bagus@ep.its.ac.id)
# changelog:
# 2020-02-12: initial code, modified from atsit code
...
vad = np.load('/home/s1820002/IEMOCAP_full_release/y_egemaps.npy')
...
# text feature
# text data
path = '/home/s1820002/IEMOCAP_full_release/'
...
def model(alpha, beta, gamma):
    ...
    net_text = CuDNNLSTM(x_train_text.shape[1], return_sequences=True)(net_text)
    net_text = CuDNNLSTM(256, return_sequences=True)(net_text)
    net_text = CuDNNLSTM(128, return_sequences=False)(net_text)
    net_text = Dense(64)(net_text)
    model_text = Dropout(0.4)(net_text)
    target_names = ('v', 'a', 'd')
    model_text = [Dense(1, name=name)(model_text) for name in target_names]
    # model_combined = Dense(1)(model_combined)
    
    model = Model(input_text, model_combined) 
...
hist = model.fit(x_train_text[:7869],
                 ...
                 callbacks=[earlystop])
metrik = model.evaluate(x_train_text[7869:], vad[7869:])
