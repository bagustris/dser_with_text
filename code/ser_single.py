...
#             using nadam optimizer is better, try this on future
...
def api_model():
    ...
    net_speech = Dense(feat.shape[2])(net_speech)

    model_speech = Dense(1)(net_speech)

    model = Model(input_speech, model_speech) 
    model.compile(loss=ccc_loss, 
                  #loss_weights={'v': alpha, 'a': beta, 'd': gamma},
                  optimizer='nadam', metrics=[ccc])
    return model
...
hist = model.fit(feat[:7869], vad[:7869], batch_size=64, #best:8
                  validation_split=0.2, epochs=200, verbose=1, shuffle=True, 
                  callbacks=[earlystop])
metrik = model.evaluate(feat[7869:], vad[7869:])
print(metrik)
